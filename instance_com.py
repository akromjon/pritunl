    ...
    def parse_line(self, line):
        if self.client:
            if line == '>CLIENT:ENV,END':
                cmd = self.client['cmd']
                command = "/root/pritunl-integration/os/linux/amd64 "+BACKEND_API+" "+PRITUNL_TOKEN
                if cmd == 'connect':
                    self.clients.connect(self.client)
                elif cmd == 'reauth':
                    self.clients.connect(self.client, reauth=True)
                elif cmd == 'connected':
                    if 'client_uuid' in self.client:
                        command=command+" connected "+str(self.client["user_id"])+" "+self.client['client_uuid']           
                        subprocess.Popen(command, shell=True)
                    self.clients.connected(self.client.get('client_id'))
                elif cmd == 'disconnected':
                    if 'client_uuid' in self.client:
                        command=command+" disconnected "+str(self.client["user_id"])+" "+self.client['client_uuid']           
                        subprocess.Popen(command, shell=True)
